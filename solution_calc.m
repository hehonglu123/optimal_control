...
while (n<=Max)&(~conv)
    ...
    J(n) = 0.5*(x(N+1,n)^2+y(N+1,n)^2+theta(N+1,n)^2);
    
    l1(N,n)=x(N+1,n); %terminal co-state
    l2(N,n)=y(N+1,n);
    l3(N,n)=theta(N+1,n);
    ...
    for k=1:N
        ...
        u1(k,n+1) = min(1,max(-1,u1(k,n+1))); %clipping the value of u to between -1 and 1
        u2(k,n+1) = min(1,max(-1,u2(k,n+1)));
    end
    if n>1
